...
class EvaluationController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Retrieve all users based on the role of the logged-in user
        $usersQuery = User::query();

        if ($currentUser->isAdmin()) {
            // If the user is an administrator, retrieve only users with evaluations
            $usersQuery->has('evaluations')->with('department');
        } elseif ($currentUser->isSupervisor()) {
            // If the user is a supervisor, retrieve only users from their department
            $usersQuery->where('department_id', $currentUser->department_id);
        } else {
            // If the user is not an admin or supervisor, retrieve their own department's users
            $usersQuery->where('department_id', $currentUser->department_id);
        }

        // Filter users by department if a department is selected
        if ($selectedDepartment) {
            $usersQuery->whereHas('department', function ($query) use ($selectedDepartment) {
